...
const port_get = process.env.PORT || 3050;
const app2 = express();
const app = express();
const server = http.createServer(app);
const server2 = http.createServer(app2);
const ZDGPath = './ZDGSessions/';
const ZDGAuth = 'auth_info_31_97609976.json';
...
const sessionClient = new dialogflow.SessionsClient({keyFilename: "amilar-best-play-ixnc-819b4f860093.json"});
   ...
   ZDGsock.ev.on('messages.upsert', async ({ messages, type }) => {
      ...
      const jid = msg.key.remoteJid
         ...
         if (!ZDGGroupCheck(jid) && !msg.key.fromMe && jid !== 'status@broadcast') {
            ...
            let textoResposta = await executeQueries("amilar-best-play-ixnc", jid, [JSON.stringify(msg.message.conversation)], 'pt-BR');
...
app2.get('/reload',(request,resposta_status)=>{
    // return resposta_status.json({message : 'servidor esta rodando tranquilo botpost'})
    const reload = exec('pm2 reload botamilar.js');
